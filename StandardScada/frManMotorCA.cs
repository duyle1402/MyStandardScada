...
{
    public partial class frManMotorCA : Form
    {
        public frManMotorCA()
        {
            InitializeComponent();
        ...
        private void frManMotor1_Load(object sender, EventArgs e)
        {
            if(PLC.Instance().PLC_Connected)
            {
              ...
              //Show_Data_Time_txt(txtSPON, PLC_DB102_RetainWrite.Instance().SP_ON_Bom_CA);
              //Show_Data_Time_txt(txtSPOFF, PLC_DB102_RetainWrite.Instance().SP_OFF_Bom_CA);
    ...
    private void timerMan_Tick(object sender, EventArgs e)
        {
            //ON_OFF_button(btnAuto, PLC_DB101Write.Instance().Auto_Man_Bom_CA); // gọi hàm trạng thái button automan
            //ON_OFF_button(btnStart, PLC_DB101Write.Instance().Start_Man_Bom_CA); // gọi hàm trạng thái button start
            //ON_OFF_button(btnStop, PLC_DB101Write.Instance().Stop_Man_Bom_CA); // gọi hàm trạng thái button stop
            //ON_OFF_ALARM(sdMotor1, PLC_DB100Read.Instance().output_Bom_CA, PLC_DB100Read.Instance().Error_Bom_CA); // gọi hàm trạng thái của bơm điều hoà
            //Show_Data_Time_txt(txtPVON, PLC_DB100Read.Instance().PV_ON_Bom_CA); // show thời gian pv on
            //Show_Data_Time_txt(txtPVOFF, PLC_DB100Read.Instance().PV_OFF_Bom_CA); // show thời gian pv off
            //Show_Data_Double_txt(txtRunTime, PLC_DB100Read.Instance().Time_Run_Bom_CA); // show thời gian run time
        ...
        private void txtSPON_KeyDown(object sender, KeyEventArgs e) 
        {
            ...
            {
                Write_Data_Time_txt("DB102.DBD8", txtSPON);
            }
        }
        ...
        private void txtSPOFF_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                Write_Data_Time_txt("DB102.DBD12", txtSPOFF);
            }
        }
